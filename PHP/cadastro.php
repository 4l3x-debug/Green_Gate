...
<body>
	...
	if (isset($_POST['cadastrar'])) {
		...
		if (empty($_POST['nome']) or empty($_POST['email']) or empty($_POST['senha']) or empty($_POST['senha-confirmacao'])  or empty($_POST['usuario']) or empty($_POST['data']) or empty($_POST['cpf']) or empty($_POST['genero'])) {
			...
			echo ('<script type="text/javascript">window.alert("Preencha os campos!");window.location="cadastro.php"</script>');
		} else {
			...
			if (!empty($foto["name"])) {
				...
				if ($usuario == 1) {
					if ($senha == $confimacao) {
						...
						if ($query_cadastro) {
							echo ('<script type="text/javascript">window.alert("Cadastro efetuado com sucesso!");window.location="login.php"</script>');
						} else {
							echo ('<script type="text/javascript">window.alert("Erro ao se cadastrar!");window.location="cadastro.php"</script>');
						}
					} else {
						echo ('<script type="text/javascript">window.alert("As senhas estão incompatíveis!");window.location="cadastro.php"</script>');
					}
				} else if ($usuario == 2) {
					if ($senha == $confimacao) {
						$adicionar = 'insert into usuario (nome, email, senha, usuario, celular, data_nascimento, data_cadastro, genero, cpf, imagem) values ("' . $nome . '", "' . $email . '", "' . md5($senha) . '", ' . $usuario . ', "' . $celular . '", "' . $data_americana . '", "' . $data_cadastro . '", "' . $genero . '", "' . $cpf . '", "' . $nome_imagem . '");';
